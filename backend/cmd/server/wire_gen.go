...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	// Redis 在途预留不可用时回落到 DB 预留表（参考 new-api 的 DB 条件更新回落）。
	billingCacheService.SetReservationFallback(repository.NewBillingReservationDBStore(db))
	...
	usageBillingRepository := repository.NewUsageBillingRepository(client, db, configConfig)
	...
	balanceNotifyService := service.ProvideBalanceNotifyService(configConfig, emailService, settingRepository, accountRepository, notificationEmailService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, emailBroadcastHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, imageCreatorStorageGovernanceHandler, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelSquareHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, imageCreatorHandler, canvasHandler, studioBridgeHandler, promptFavoriteHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
