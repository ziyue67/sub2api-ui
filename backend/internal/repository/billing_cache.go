...
var (
	...
	// 否则同一笔请求会被记两次额度。
	//
	// 聚合键的 TTL 只在「键新建 / 无 TTL」时设置，绝不在每次预留时无条件续期。
	//
	// 原因（这是本脚本最容易写错的一处）：放行判定的顺序是「先落凭据+累加 → 脚本返回
	// 后再在 Go 侧校验封底护栏 → 护栏不成立就调用 release 回滚本笔」。也就是说**被拒绝
	// 的请求也会完整走完这个脚本**（唯一的区别是随后被回滚）。回滚只归还金额、不会归还
	// TTL；若这里无条件 PEXPIRE，那么每一次被拒绝的请求都会把「历史泄漏」
	// （进程崩溃 / 结算任务被丢弃 / 晚到归还被忽略而从未递减的聚合金额）的过期时间
	// 重新续满。并发突发时拒绝量远大于放行量，泄漏被反复续期、永不消失，用户会在余额
	// 远高于封底时就因"在途预留"被持续 403，而且只要还有流量就永远无法自愈。
	//
	// 正确语义：泄漏金额的存活期以聚合键最后一次"真正建立"为起点按 TTL 自然衰减；
	// 键尚在（PTTL >= 0）时不再续期，因此突发结束后最多 TTL 时间即自行过期、自愈。
	//
	// 已知取舍：极端情况下若同一个用户连续 TTL 时长一直保持有在途请求，聚合键可能在
	// 仍有在途请求时过期，使护栏短暂少算，从而多放行几笔；此时结算侧
	// GREATEST(balance - amount, floor) 仍然兜底，余额绝不为负，最多产生有界的 shortfall。
	// 这比"泄漏永不消失、账号在封底之上被永久锁死"要好得多；彻底消除该取舍需要把护栏
	// 判定下沉进本脚本（被拒绝的请求根本不落凭据、不碰聚合键），留作后续改造。
	//
