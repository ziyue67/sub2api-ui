...
// The built-in pin is intentionally bumped as upstream model gates change.
// Keep the override tests relative to that pin so they verify the contract
// instead of becoming stale whenever CLICurrentVersion advances.
func nextPatchVersion(version string) string {
	parts := strings.Split(version, ".")
	if len(parts) != 3 {
		panic("CLICurrentVersion must be a three-part version")
	}
	patch, err := strconv.Atoi(parts[2])
	if err != nil {
		panic("CLICurrentVersion patch must be numeric")
	}
	return fmt.Sprintf("%s.%s.%d", parts[0], parts[1], patch+1)
}

func TestIsSupportedCLIVersion(t *testing.T) {
	supportedOverride := nextPatchVersion(CLICurrentVersion)
	cases := []struct {
		name    string
		version string
		want    bool
		...
		{"高于基线的补丁位", supportedOverride, true},
		{"高于基线的次版本", "2.2.0", true},
		{"高于基线的主版本", "3.0.0", true},
		{"低于基线", "2.1.219", false},
		{"远低于基线", "1.0.0", false},
...
func TestResolveCLIVersion(t *testing.T) {
	...
	cases := []struct {
		...
		{"合法覆盖生效", supportedOverride, supportedOverride},
		{"两侧空白被裁掉", "  " + supportedOverride + "  ", supportedOverride},
