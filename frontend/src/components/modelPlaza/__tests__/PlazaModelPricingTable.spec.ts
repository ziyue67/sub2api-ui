...
describe('PlazaModelPricingTable', () => {
  ...
  it('长阶梯标签与价格分层展示,不使用整行不换行布局', () => {
    ...
    const wrapper = mountTable([model], 0.9)
    const entries = wrapper.findAll('.pz-tier-entry')

    expect(entries).toHaveLength(2)
    expect(entries.every((entry) => !entry.classes().includes('whitespace-nowrap'))).toBe(true)
    expect(entries.map((entry) => entry.find('.pz-tier-label').text())).toEqual([
      'UI1 Demo base',
      'UI1 Demo base'
    ])
    expect(entries.map((entry) => entry.find('.pz-tier-value').text())).toEqual([
      '$2.70',
      '$13.50'
    ])
  })
  ...
  it('Composite 分组中相同模型名按具体平台分别展示 UI1 徽章', () => {
    const anthropic = tokenModel({ name: 'shared-model', platform: 'anthropic' })
    const openai = tokenModel({ name: 'shared-model', platform: 'openai' })
    const antigravity = tokenModel({ name: 'shared-model', platform: 'antigravity' })
    const gemini = tokenModel({ name: 'shared-model', platform: 'gemini' })
    const wrapper = mount(PlazaModelPricingTable, {
      props: {
        models: [anthropic, openai, antigravity, gemini],
        platform: 'composite',
        rateMultiplier: 1
      }
    })
    ...
    expect(rows.map((row) => row.find('td').text())).toEqual(
      expect.arrayContaining([
        ...
        'shared-modelOpenAI',
        'shared-modelAntigravity',
        'shared-modelGemini'
      ])
    )

    const badges = wrapper.findAll('.pz-platform-badge')
    expect(badges).toHaveLength(4)
    expect(badges.map((badge) => badge.text())).toEqual(
      expect.arrayContaining(['Anthropic', 'OpenAI', 'Antigravity', 'Gemini'])
    )
    for (const badge of badges) {
      expect(badge.attributes('class')).not.toMatch(/(?:bg|text)-(?:purple|blue)-/)
    }
  })
})
