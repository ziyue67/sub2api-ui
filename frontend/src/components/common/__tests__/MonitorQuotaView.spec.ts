...
describe('MonitorQuotaView', () => {
  ...
  it('maps tier window/label tokens through i18n and colors utilization', () => {
    ...
    const rows = wrapper.findAll('[data-testid="monitor-quota-view"] [data-testid="monitor-quota-tier"]')
    ...
    // 阈值逻辑来自上游 UsageProgressBar，但渲染使用 Scheme3 语义皮肤。
    expect(html).toContain('scheme3-usage-bar-healthy')
    expect(html).toContain('scheme3-usage-bar-warning')
    expect(html).toContain('scheme3-usage-bar-critical')
    expect(html).not.toMatch(/(?:bg|text)-(?:emerald|amber|red)-/)
  })
  ...
  it('shows the plan level badge and multi-currency balances', () => {
    ...
    // 余额为 0 使用 Scheme3 的 critical 语义警示。
    expect(wrapper.html()).toContain('is-empty')
  })
