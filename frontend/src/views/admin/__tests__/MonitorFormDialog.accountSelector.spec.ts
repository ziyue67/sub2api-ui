...
const selectPortalSelector = '.select-dropdown-portal, .scheme3-select-dropdown-portal'
const selectOptionSelector = '.select-option, .scheme3-select-option'
const selectSearchSelector = '.select-search-input, .scheme3-select-search-input'

const openAccountDropdown = async (wrapper: VueWrapper) => {
  await accountTrigger(wrapper).trigger('click')
  await nextTick()
  const dropdown = document.body.querySelector<HTMLElement>(selectPortalSelector)
  expect(dropdown).not.toBeNull()
  return dropdown as HTMLElement
}

const clickOption = (dropdown: HTMLElement, text: string) => {
  const option = [...dropdown.querySelectorAll(selectOptionSelector)].find((el) =>
...
const typeAccountSearch = async (dropdown: HTMLElement, query: string) => {
  const input = dropdown.querySelector<HTMLInputElement>(selectSearchSelector)
