...
- **保守方向的误伤**：输入 token 按**三段口径**估计 —— 普通文本 `字节数/2`；
  **多模态块内**的 base64（`url`/`data`/`file_data` 字段承载，即真实图片/音频负载）
  按固定 allowance（1600/块，与分辨率无关）；**文本内容里**的 base64（不处于多模态块内，
  判定依据是负载标记前最近的 JSON 键名）按 **1 token/字节** 稠密计
  （实测 20KB base64 被上游分词为 18907 token ≈ 0.92 token/字节，若按图片折算会低估 4 倍、
  生产实测产生过一笔 $0.0034 的 write-off）。对超长单行文本等异常内容仍会高估。
